...
class Jetpack_Omnisearch_Posts extends WP_List_Table {
	...
	function get_columns() {
		$columns = array(
		#	'id' => __('ID', 'jetpack-omnisearch'),
			'post_title' => __('Title', 'jetpack-omnisearch'),
			'snippet' => __('Snippet', 'jetpack-omnisearch'),
			'date' => __('Date', 'jetpack-omnisearch'),
		);
		return $columns;
	}
	...
	function column_post_title( $post ) {
		...
		if ( current_user_can( $this->post_type_obj->cap->delete_post, $post ) ) {
			$actions['delete'] = sprintf( '<a href="%s">%s</a>', esc_url( get_delete_post_link( $post->ID ) ), esc_html__('Trash', 'jetpack-omnisearch') );
		}
	...
	function column_date( $post ) {
		...
		if ( '0000-00-00 00:00:00' == $post->post_date ) {
			$t_time = $h_time = __('Unpublished', 'jetpack-omnisearch');
			$time_diff = 0;
		} else {
			$t_time = date( __('Y/m/d g:i:s A', 'jetpack-omnisearch'), mysql2date( 'G', $post->post_date ) );
			...
			if ( $time_diff > 0 && $time_diff < DAY_IN_SECONDS )
				$h_time = sprintf( __('%s ago', 'jetpack-omnisearch'), human_time_diff( $time ) );
			else
				$h_time = mysql2date( __('Y/m/d', 'jetpack-omnisearch'), $m_time );
		}

		$html .= '<abbr title="' . esc_attr( $t_time ) . '">' . esc_html( $h_time ) . '</abbr>';
		$html .= '<br />';
		if ( 'publish' == $post->post_status ) {
			$html .= esc_html__('Published', 'jetpack-omnisearch');
		} elseif ( 'future' == $post->post_status ) {
			if ( $time_diff > 0 )
				$html .= '<strong class="attention">' . esc_html__('Missed schedule', 'jetpack-omnisearch') . '</strong>';
			else
				$html .= esc_html__('Scheduled', 'jetpack-omnisearch');
		} else {
			$html .= esc_html__('Last Modified', 'jetpack-omnisearch');
		}
		return $html;
	}
