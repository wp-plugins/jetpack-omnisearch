...
class Jetpack_Omnisearch_Media extends WP_Media_List_Table {
	...
	function search( $results, $search_term ) {
		...
		$search_link = sprintf( ' <a href="%s" class="add-new-h2">%s</a>', $search_url, esc_html__('Search Media', 'jetpack-omnisearch') );
		$html = '<h2>' . esc_html__('Media', 'jetpack-omnisearch') . $search_link . '</h2>';
		parent::__construct();
		...
		$label = __( 'Media', 'jetpack-omnisearch' );
		$results[ $label ] = $html;
		return $results;
	}
