...
class Jetpack_Omnisearch {
	...
	function omnisearch_page() {
		...
		?>
		<div class="wrap">
			<h2 class="page-title"><?php esc_html_e('Omnisearch', 'jetpack-omnisearch'); ?> <small><?php esc_html_e('search everything', 'jetpack-omnisearch'); ?></small></h2>
			<br class="clear" />
			...
			<?php if( ! empty( $results ) ): ?>
				<h3 id="results-title"><?php esc_html_e('Results:', 'jetpack-omnisearch'); ?></h3>
				<div class="jump-to"><strong><?php esc_html_e('Jump to:', 'jetpack-omnisearch'); ?></strong>
				...
				<ul class="omnisearch-results">
					<?php foreach( $results as $label => $result ) : ?>
						<li id="result-<?php echo sanitize_title( $label ); ?>" data-label="<?php echo esc_attr( $label ); ?>">
							<?php echo $result; ?>
							<a class="back-to-top" href="#results-title"><?php esc_html_e('Back to Top &uarr;', 'jetpack-omnisearch'); ?></a>
						</li>
					<?php endforeach; ?>
				</ul>
			<?php endif; ?>
	...
	static function get_omnisearch_form( $args = array() ) {
		$defaults = array(
			...
			'search_placeholder' => __( 'Search Everything', 'jetpack-omnisearch' ),
			'submit_class'       => 'button',
			'submit_value'       => __( 'Search', 'jetpack-omnisearch' ),
