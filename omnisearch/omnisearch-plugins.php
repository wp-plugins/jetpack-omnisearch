...
class Jetpack_Omnisearch_Plugins extends WP_Plugin_Install_List_Table {
	...
	function search( $results, $search_term ) {
		...
		$search_link = sprintf( ' <a href="%s" class="add-new-h2">%s</a>', $search_url, esc_html__('Search Plugins', 'jetpack-omnisearch') );
		$html = '<h2>' . esc_html__('Plugins', 'jetpack-omnisearch') . $search_link . '</h2>';

		$html .= '<div id="' . __CLASS__ . '_results">' . esc_html__('Loading &hellip;', 'jetpack-omnisearch') . '</div>';
		...
		$label = __( 'Plugins', 'jetpack-omnisearch' );
		$results[ $label ] = $html;
		return $results;
	}
