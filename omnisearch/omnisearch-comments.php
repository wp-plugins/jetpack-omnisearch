...
class Jetpack_Omnisearch_Comments extends WP_Comments_List_Table {
	...
	function search( $results, $search_term ) {
		...
		$search_link = sprintf( ' <a href="%s" class="add-new-h2">%s</a>', $search_url, esc_html__('Search Comments', 'jetpack-omnisearch') );
		$html = '<h2>' . esc_html__('Comments', 'jetpack-omnisearch') . $search_link . '</h2>';
		parent::__construct();
		...
		$label = __( 'Comments', 'jetpack-omnisearch' );
		$results[ $label ] = $html;
		return $results;
	}
